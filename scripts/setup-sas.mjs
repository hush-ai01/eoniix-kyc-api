...
const createCredentialIx = getCreateCredentialInstruction({ payer: issuer, credential: credentialPda, authority: issuer, name: CREDENTIAL_NAME, signers: [issuer.address] });
const createSchemaIx = getCreateSchemaInstruction({ payer: issuer, credential: credentialPda, schema: schemaPda, authority: issuer, name: SCHEMA_NAME, fieldNames: ['eNumber', 'country', 'idType', 'amlClear', 'verifiedAt'], fieldTypes: [0, 0, 0, 3, 0] });

const client = createSolanaClient({ urlOrMoniker: 'devnet' });
const { rpc, sendAndConfirmTransaction } = client;
const { value: latestBlockhash } = await rpc.getLatestBlockhash().send();

const tx = createTransaction({ version: 0, feePayer: issuer, instructions: [createCredentialIx, createSchemaIx], latestBlockhash });
